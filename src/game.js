...
export default class SolitaireGame{
  ...
  reset(){
    ...
    // ensure faceDown/faceUp for tableau deal
    this.foundations = [[],[],[],[]];
    this.waste = [];
    this.stock = d.slice();
    this.tableau = Array.from({length:7},()=>[]);
    // deal tableau: 1..7, last card faceUp
    for(let i=0;i<7;i++){
      for(let j=0;j<=i;j++){
  ...
  canPlaceOnTableau(card, destPile){
    if(destPile.length===0) return rankValue(card.rank)===13; // King onto empty
  ...
  // move a sequence of cards from tableau[fromIndex] starting at cardIndex to tableau[toIndex]
  ...
  // try to auto-move any valid foundation moves from waste/top of tableau
  autoMoveToFoundations(){
    // simple greedy: try waste, then tableau tops
    let moved = true;
    while(moved){
      moved = false;
      ...
      for(let t=0;t<7 && !moved;t++){
        if(this.tableau[t].length){
          const card = this.tableau[t][this.tableau[t].length-1];
          for(let f=0;f<4;f++){
