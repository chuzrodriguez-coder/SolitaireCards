export function renderGame(game){
  ...
  for(let i=0;i<4;i++){
    ...
    if(game.foundations[i].length){
      ...
      // foundation uses an <img> element for the card face
      el.textContent = '';
      const img = document.createElement('img');
      img.src = `cards/${card.id}.png`;
      img.alt = card.id;
      el.appendChild(img);
      // set text color to keep contrast, though image covers it
  ...
  game.tableau.forEach((pile,pi)=>{
    ...
    pile.forEach((c,idx)=>{
      ...
      if(c.faceUp){
        // insert img instead of background
        card.textContent = '';
        const img = document.createElement('img');
  ...
  if(game.waste.length){
    ...
    // set red for red suits, black for others; clear if empty later
  ...
  // progress: foundation completion + longest face-up descending sequences per tableau
  progress.innerHTML = '';
  ...
  game.tableau.forEach((pile,pi)=>{
    // compute longest descending alternating color sequence from the first faceUp card from top
    let best = 0;
    for(let i=0;i<pile.length;i++){
      ...
      for(let j=i+1;j<pile.length;j++){
        ...
        // descending rank and alternating color
        const prevVal = Number(prev.rank==='A'?1:(prev.rank==='J'?11:(prev.rank==='Q'?12:(prev.rank==='K'?13:prev.rank))));
        const curVal = Number(cur.rank==='A'?1:(cur.rank==='J'?11:(cur.rank==='Q'?12:(cur.rank==='K'?13:cur.rank))));
